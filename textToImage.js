...
async function handleDallERequest(request) {
    try {
        ...
        if (!(serviceRequest.prompt)) throw new Error('Invalid DALL-E Request format');
        ...
        if (response.data) {
            return { mediaUrl: response.data.data[0].url, caption: (serviceRequest.caption ? serviceRequest.caption : serviceRequest.prompt), type: 'media', from: 'dall-e' };
        } else {
